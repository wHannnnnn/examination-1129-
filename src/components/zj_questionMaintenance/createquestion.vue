  ...
  <div>
      ...
      <el-tabs type="border-card">
        <el-tab-pane label="人工录入">
            <el-form :model="directoryCreate" :rules="rules" status-icon ref="directoryCreate" label-width="50px" class="demo-ruleForm" size="mini" style="height: 60px">
              <el-row>
                <el-col :span=5>
                  <el-form-item>
                    <el-cascader
                      ...
                      filterable
                      change-on-select
                      @change="pointsInvolve">
                    </el-cascader>
                  </el-form-item>
                </el-col>
                <el-col :span=5>
                  <el-form-item prop="question_ability_norm">
                    <el-select v-model="directoryCreate.question_ability_norm"  filterable placeholder="选择能力指标">
                ...
                <el-col :span=4>
                  <el-form-item prop="exam_modality">
                    <el-select v-model="directoryCreate.exam_modality"  filterable placeholder="选择用途">
                ...
                <el-col :span=5>
                  <el-form-item  prop="questionsTestingType">
                    <!-- 选择题型 -->
                    <el-select v-model="directoryCreate.questionsTestingType" placeholder="请选择题型">
                      <el-option
                        v-for="item in initData.question_type"
                        :key="item.value"
                        :label="item.dictName" 
                        :value="item.value">
                      </el-option>
                ...
                <el-col :span=5>
                  <el-form-item prop="question_difficulty">
                    <el-select v-model="directoryCreate.question_difficulty" placeholder="请选择试题难度">
                      <el-option
                ...
                <el-row>
                  <el-col :offset='1'>  
                    <el-button size="mini" round v-show='chakan' @click='involveShow'>{{msg}}</el-button>

                    <el-tag
                      v-show='neirong'
                      v-for="tag in involveItem"
                      :key="tag.name"
                      closable
                    ...
                    <el-button class="button-new-tag" size="mini" v-show='neirong' @click="showInput">添加</el-button>
                  </el-col>
                </el-row>    
            </el-form>

              <el-dialog title="知识点" :visible.sync="graphEditDel">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;请您根据您的上传文件格式选择对应的上传文档模版，先进行模版下载，并详细阅读模版说明，之后再按照模版格式要求对自己相应的文档进行排版，最后再将自己要上传的文档进行上传。
                  ...
                  <div class="xiazai">
                    <el-row>
                      <el-col :span="7">
                        <el-button size="medium" type="success" @click="excelxiazai" icon="el-icon-download">下载EXCEL格式模板</el-button>
                      </el-col>
                      <el-col :span="7">
              ...
              <div class="xzmb">
                ...
                <div class="neirong">
                  ...
                  <el-row>
                    ...
                    <el-select v-model="selectvalue" placeholder="请选择试题类型"  size="mini">
                    ...
                    <el-select v-model="examModality"  filterable placeholder="选择试题难度" size="mini">
                      <el-option v-for="(item, index) in initData.question_difficulty"
                        :key="index" :label="item.dictName" :value="item.value">
                      </el-option>
                    </el-select>
                ...
                <div class="biaodan"  v-if="this.uploadTable.length>0">
                    ...
                    <el-row>
                      <el-col :span="7">
                        <el-upload
                          ...
                          accept=".xls,.xlsx"
                          >
                          <el-button size="medium" type="success">导入EXCEL格式模板</el-button>
                        </el-upload>
                      </el-col>
                      <el-col :span="7">
                        <el-upload
...
import sjdr from '@/components/zj_questionMaintenance/sjdr.vue'
...
export default {
  data(){
      return{
        ...
        uploadTable: [],
        //   初始化数据
        uploadUrl: this.$axiosURL.fm_fileManipulate + '/upload/bashImport/wordOrExcel' + '/' +this.$store.state.aId + '/' + 1,
        initData: {},
        inputVisible: false,
        neirong: false,
        chakan:false,
...
//      附件数据集合
  ...
  methods: {
    ...
    doFilter () {      
      ...
      //   if(this.uploadTable[i].simulateOrOfficial == this.examModality) {
      //     console.log()
      //   }
      // }
      this.tableDataEnd = []
      this.filterTableDataEnd = []
      // this.uploadTable.forEach((value, index) => {
      //   console.log(value,22222222)
      //   if (value.questionBody) {
      //     if (this.selectvalue === '') {
      ...
      // if(this.examModality !== '') {
      //   console.log('aaaaaaaaaa')
      //   // 判断检索条件
      // }
      if(this.selectvalue!=='' || this.examModality!=='') {
        // if(this.examModality == '模拟考试') {
        //   this.examModalitys = false
        // } else if(this.examModality == '正规考试'){
        //   this.examModalitys = true
        // }
        var asubjectsIdsStrs1 = this.subjectsIdsStrs1[this.subjectsIdsStrs1.length-1]

        var test = this.uploadTable.filter((a)=> {
          console.log(a.questionDifficulty,33)
          if(this.examModality !== '' && this.selectvalue == '') {
            return a.questionDifficulty == this.examModality
          }else if(this.examModality == '' && this.selectvalue !== '') {
            return a.questionType == this.selectvalue
          }else if(this.examModality !== '' && this.selectvalue !== '') {
            return a.questionType == this.selectvalue && a.questionDifficulty == this.examModality
          }
        })

        console.log(this.uploadTable,this.examModality,this.selectvalue)
        this.tableDataEnd = test
        this.filterTableDataEnd = this.tableDataEnd
      }
    ...
    //   this.tableDataEnd = []
    //   this.filterTableDataEnd = []
    //   this.uploadTable.forEach((val, index) => {
    //     if (val.questionsTestingTypesName) {
    //       if (val.questionsTestingTypesName.indexOf(this.selectvalue) >= 0 && val.questionsTopic.indexOf(this.tableDataName) >= 0) {
    //         this.filterTableDataEnd.push(val)
    //       }
    //     }
    //   })
    //   if (this.selectvalue === '所有题型') {
    ...
    // uploadUrl(){
    //   var url = this.$axiosURL.fm_fileManipulate + '/upload/bashImport/wordOrExcel'
    //   return url
    // },、
    ...
    excelxiazai(){
      var url = this.$axiosURL.fm_fileManipulate + '/download/model/'+ '新考试系统批量上传试题excel模版.xlsx'
      window.location.href = url
    },
    wordxaizai(){
      var url =this.$axiosURL.fm_fileManipulate + '/download/model/'+ '试题批量上传Word模版与说明.zip'
      window.location.href = url
    },
    ...
    graphAddSubmit(){
      var _this = this;
      if(this.subjectsIdsStrs.length>1){
        Utils4.MSDataTransfer.treeToArray(this.options,this.xuekeChangItems)
        // 拼接 页面
        for (var i = 0; i < this.xuekeChangItems.length; i++) {
          this.involveItem.push(this.xuekeChangItems[i])
        }
        this.graphEditDel = false
        this.garphAddShow = false
        this.form.isp1 =[]
        ...
        // 学科 选中的数据拼接到展示数据上 相关数据移除
        var arr = []
        for (var i = 0; i < this.options.length; i++) {
          for (var j = 0; j < this.xuekeChangItems.length; j++) {
            if(this.options[i].id == this.xuekeChangItems[j].id) {
              arr.push(i)
            }
          }
        }
        for (var i = 0; i < arr.length; i++) {
          delete this.options[arr[i]]
        }
        var arrr=this.options.filter(item => item)
        this.options = arrr
        for (var i = 0; i < this.xuekeChangItems.length; i++) {
          // 拼接
          this.involveItem.push(this.xuekeChangItems[i])
    ...
    handleClose(tag) {
      this.involveItem.splice(this.involveItem.indexOf(tag), 1)
      if(tag.khId){
        this.delItems.push(tag)
      } else{
        Utils3.MSDataTransfer.treeToArray(this.options,tag)
        this.graphEditDel = false
        this.garphAddShow = false
    ...
    showInput() {
      if(this.subjectsIdsStrs.length>1){
        ...
        // params['kpId'] = Number(this.directoryCreate.subjectsIdsStr)
        // this.$axiosRes('get',this.$axiosURL.k_knowledgePoint+ '4/find/nonParentHierachies/and/excludeHaveInvolvedPoints/' + Number(this.subjectsIdsStrs[this.subjectsIdsStrs.length - 1])).then((res)=>{
        //   this.options = res
        //   for (var i = 0; i < this.options.length; i++) {
        //     this.options[i]['disabled'] = true
        //   }
        ...
        for (var i = 0; i < this.delItems.length; i++) {
          this.options.push(this.delItems[i])
        }
        this.delItems = []
      }
    ...
    handleInputConfirm() {
      ...
      // if (inputValue) {
      //   this.involveItem.push(inputValue);
      // }
    ...
    pointsInvolve(val){
      ...
      if(this.subjectsIdsStrs.length>1){
        // 知识点
        // params['kpId'] = Number(this.directoryCreate.subjectsIdsStr)
        this.$axiosRes('get',this.$axiosURL.k_knowledgePoint +'find/'+ Number(this.subjectsIdsStrs[this.subjectsIdsStrs.length - 1]) + '/correlatedPoint').then((res)=>{
          this.involveItem = res.correlateKnowledgePoints
          this.chakan = true
          Utils5.MSDataTransfer.treeToArray(this.options,res.name)
          Utils4.MSDataTransfer.treeToArray(this.options,this.involveItem)
        })
      } else {
        // 学科
        this.$axiosRes('get',this.$axiosURL.k_knowledgeHierachy +'find/' + Number(this.subjectsIdsStrs) + '/involvedHierachy').then((res)=>{
          this.involveItem = res.involveKnowledgeHierachies
          this.chakan = true
        })
      }
       ...
       if(this.subjectsIdsStrs.length>1){
        ...
        // 学科
        this.$axiosRes('get',this.$axiosURL.k_knowledgeHierachy+ '0/find/'+ Number(this.subjectsIdsStrs) +'/theOtherAll').then((res)=>{
          this.options = res
          // this.options.push(this.delItems)
          // if(this.)
        })
      }
    },
    involveShow(){
      if(this.neirong == true) {
        this.neirong = false
        this.msg = '关联知识点查看'
      } else {
        this.neirong = true
    ...
    handleSubmitData(formName){
        ...
        this.$refs['directoryCreate'].validate((valid) => {
          if (valid) {
            ...
            var params ={
              ...
              examinationQuestion:{
                ...
                simulateOrOfficial: this.directoryCreate.exam_modality,
            ...
            if(this.subjectsIdsStrs.length>1){
              ...
              // 学科
              params['khId'] = Number(this.directoryCreate.subjectsIdsStr)
    ...
    handleSubmitDataOne(formName){
          // if(formName.exam_modality =='模拟训练'){
          //   formName.simulateOrOfficial = false
          // } else {
          //   formName.simulateOrOfficial = true
          // }
