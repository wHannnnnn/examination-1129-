        ...
        <el-table-column label="操作" width="150" :key="Math.random()" >
          <template slot-scope="scope">
            <el-button
              size="mini"
              type="danger"
              @click="clickKSPJ(scope.$index, scope.row)">试卷归档
            </el-button>
          </template>
        </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
      ...
      findPaperPage:function(){
        ...
        this.$axiosRes1('post',this.$axiosURL.e_examination+'findChiefingByParam',param).then((res)=>{
